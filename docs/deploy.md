...
**Вход и регистрация — один шаг** (паттерн sign-in-or-up, как у Clerk, Supabase и Firebase). Код уходит на любой корректный адрес, и экран для знакомой и незнакомой почты одинаковый, поэтому перебором адресов нельзя узнать, кто обращался к психологам. Развилка происходит после того, как человек доказал, что почта его: есть аккаунт — ведём в кабинет, нет — заводим аккаунт и ведём в анкету, где почта уже подтверждена и спрашивать её второй раз не нужно. Коды для адресов без аккаунта лежат в отдельной таблице `email_codes` (живут сутки) — мусорные записи в `accounts` от чужого перебора не появляются.

Аккаунт заводится сам при первом обращении (анкета, заявка психолога, бронь из каталога), но **сессию без кода получает только владелец адреса**: либо почта нам незнакома и аккаунт создаётся прямо сейчас, либо человек уже вошёл под ней. Если адрес занят чужим аккаунтом — заявка сохраняется, а вместо кабинета показывается «войдите по коду». Без этого правила достаточно было бы вписать чужую почту в анкету, чтобы попасть в чужой кабинет.
